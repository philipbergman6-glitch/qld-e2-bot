...
2. **Check the decision was mechanical.** Re-run
   ...
   *Environment for that re-run* (e2bot-19): install
   `pip install -r requirements.txt` — a tested range, not a freeze — or
   `requirements.lock` for the operator's exact set. The range is wide because
   it was measured to be safe: the rule produces bit-identical output under
   pandas 1.5.3 / 2.2.3 / 3.0.5 on Python 3.11 / 3.13 / 3.14 (0 mismatches over
   4,786 reference days in each). You do not have to take that on trust —
   `python3 -m unittest discover -s tests` re-derives the frozen rule against
   ...
   push. That check is what makes "the frozen E2 rule" a testable claim rather
   than a description.
...
- **The environment each historical signal ran under is not recorded.**
  ...
  in the signal record (e2bot-19 part 4, not yet shipped).
